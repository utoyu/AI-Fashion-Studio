...
- **Global Header Standardization:** Unified dimensions (`h-20`), padding, and typography across all dashboard functional modules. Added defensive padding (`pr-[400px]`) to all functional headers to prevent overlap with the global floating quota and theme switchers. Updated navigation hierarchies and unified iconography globally across Landing, Navbar, Sidebar, and Footers.
...
- **Architectural Separation of Quick Tools (March 2026):** Deprecated the monolithic "Smart Retouch" interface into an **Archive Center**. Replaced it with two highly specialized pages: **"One-Click Retouch"** (for single-source rapid outputs like BG removal) and **"Manual Retouch"** (dual-upload workflows for precision control, magazine splicing, and face/pose swapping).
- **Universal Theme Consistency (March 2026):** Performed a holistic audit of UI colors across the entire dashboard (KOC, Photo Studio, Model Generation). Extirpated hardcoded legacy hex codes (purples, blues, oranges) and synced all primary buttons, active borders, and highlight states exclusively to the standardized Tailwind `primary` (cyan) palette to ensure a cohesive, premium brand feeling.
